...
| **Roles** | One agent per session. Role prompts and skills are yours to write and maintain. | Six role agents (PM, architect, backend, frontend, mobile, QA) with seeded skills, rules, tool policies and memory. |
...
| **Usage limits** | When the subscription limit hits, the session stops. You come back later and re-orient it. | The task parks on Blocked and resumes the same session with --resume when the limit resets, keeping what it already read and wrote. |
