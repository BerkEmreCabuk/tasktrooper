...
| `projects/DeploySettingsSection` | Organism: one repository's whole deploy story — the scope picker (repository or sub-project), hosting link, per-environment targets, incident policy, test strategy, env inventory. Mounted twice: the settings screen's Deploy tab and the standalone `/repositories/:id/deploy` page the operations matrix links to. |
| `projects/hosting/HostingPanel` | Organism: where one scope ships to. Owns the provider choice — Vercel, Google Cloud, AWS next — and mounts one provider body at a time; a mobile scope gets the store notice instead. The scope comes from `DeploySettingsSection`, which drives the addresses with the same one. New providers are added here, never as another card on the page. |
| `projects/hosting/VercelHostingBody` | Organism: the Vercel half — linked project, production URL, latest deployment and the last failed one kept separately. Frameless; composes `VercelProjectPickerDialog`. |
| `projects/hosting/GCloudHostingBody` | Organism: the Google Cloud half — bound Cloud Run service or GKE cluster, its revisions, traffic and node pools. Frameless; composes `GCloudResourcePickerDialog`. |
| `projects/hosting/HostingFields` | Molecules shared by the provider bodies: `Field` (one labelled fact) and `ExternalLinkText` (an outward link that says it opens elsewhere). |
