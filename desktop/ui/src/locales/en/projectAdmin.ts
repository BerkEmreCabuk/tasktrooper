...
export const projectAdmin = {
  projectSettings: {
    ...
    requireReviewChain: "Require review chain before done",
    requireReviewChainHelp:
      "When on, a task can't reach done (or released, skipping done) until it has actually passed every review stage its type requires — code review, QA and PM UAT for task/bug, analiz review for analiz — and none of those stages' latest visit ended in a rejection. Off by default: a repo with no QA agent watching in_qa, or a customized board missing that column, would leave every task stuck waiting on a review it can never pass.",
    requireReleaseDeploy: "Require production deploy before released",
    requireReleaseDeployHelp:
      "When on, a task can't reach released until a production deploy has actually succeeded for it. Analiz tasks are exempt — they ship no code. Off by default: a repo with no production deploy workflow mapped records its deploy as skipped, which doesn't count as evidence, so turning this on without one will park every task in done and none will ever reach released.",
    requirePipelineForReview: "Wait for CI before code review",
    requirePipelineForReviewHelp:
      "When on (the default), a task entering code review waits for the build/test pipeline before the reviewing architect is dispatched — nobody reviews a diff that does not compile. Turn it OFF when this repository's CI cannot answer: no GitHub Actions minutes left, or checks the platform cannot read. Review then starts immediately and the card says no build stands behind it. Even with it on, the wait is bounded: if nothing reports within the gate window, or GitHub says the run cannot happen at all, the reviewer is dispatched anyway and the card records why.",
    requireOverallCoverage: "Gate on overall coverage",
    requireOverallCoverageHelp:
      "Coverage of the lines a change WRITES is always enforced at 90% — a task whose own diff is untested cannot reach code review, so no pull request opens for it. This switch is about the repository's OVERALL figure, which is off by default: overall coverage describes the codebase rather than the change, so gating on it in a repo sitting at 44% refuses every task no matter how well tested its diff is, which does not raise coverage — it stops the work. Leave it off and the number is still measured and reported on every run; turn it on once the repo can clear the bar.",
    coverageThreshold: "Overall coverage threshold (%)",
    coverageThresholdHelp:
      "The overall percentage a task must leave behind once the gate above is on. Blank uses the 90% default. Set it just above the figure the repo measures today and raise it as coverage climbs — a bar the codebase cannot reach parks every task.",
    ...
    webhookMissing:
      "No GitHub push webhook is set up: the index only refreshes when you re-index manually. Set up the webhook so every push updates the index automatically.",
    webhookSetup: "Set up webhook",
    webhookSettingUp: "Setting up…",
    webhookActive: "Push webhook active — the index refreshes automatically on every push to the default branch.",
    webhookSetupDone: "GitHub webhook set up",
    webhookSetupFailed: "Failed to set up webhook",
    ...
    testGates: "Test gates",
    testGatesInheritHint: "Untouched fields follow the repository-level setting.",
    gateInherited: "inherited",
    coverageGate: "Coverage gate",
    mutationGate: "Gate on mutation score",
    mutationGateHelp:
      "When on, a task is blocked until the repository's mutation score clears the bar below. Off by default: mutation testing is slow and most repos have no runner wired up yet — the score is still measured and reported whenever the mutation-test job runs.",
    mutationThreshold: "Mutation score threshold (%)",
    mutationThresholdHelp: "Blank uses the server default.",
    ...
    deployPackages: {
      ...
      subtitle:
        "Release trains: a set of tasks shipped together, in their deploy-dependency order. This is the release path for a repository that has automatic release on Done turned off.",
      empty: "No deploy packages yet.",
      loadFailed: "Failed to load deploy packages",
      created: "Deploy package created",
      nameLabel: "Name",
      namePlaceholder: "e.g. Sprint 14 release",
      descriptionLabel: "Description",
      descriptionPlaceholder: "What ships in this train...",
      create: "Create package",
      memberCount: "{count} task(s)",
      membersLabel: "Tasks in this package",
      noTasks: "This repository has no tasks yet.",
      editMembers: "Edit tasks",
      saveMembers: "Save tasks",
      membershipSaved: "Package tasks saved",
      release: "Release",
      releaseStarted: "Release started — the first wave has been dispatched.",
      releaseFailed: "Failed to start the release",
      releaseConfirmTitle: "Release this package?",
      releaseConfirmDesc:
        'Release "{name}" with its {count} task(s)? Production deploys are dispatched in dependency order; each task still passes its own release gates.',
  ...
  gcloudResource: {
    ...
    subtitle: "Which Cloud Run service or GKE cluster this scope runs on, and how it looks right now.",
    notBoundTitle: "No Google Cloud resource bound",
    notBoundDesc:
      "Bind the Cloud Run service or GKE cluster this scope runs on to see its revisions, traffic and health here.",
    ...
    location: "Region",
    notReported: "Not reported",
    noDetail: "This resource type is not one this version of the app can show details for.",
    openInNewTab: "opens in a new tab",
    types: {
    ...
    cloudRun: {
      ...
      noServiceUrl: "No URL reported",
      latestReady: "Last ready revision",
      latestCreated: "Last created revision",
      image: "Running image",
      updated: "Last updated",
      traffic: "Traffic split",
      trafficEmpty: "No traffic split reported.",
      readyStates: {
        ready: "Ready",
        notReady: "Not ready",
        pending: "Settling",
    ...
    gke: {
      title: "Cluster",
      masterVersion: "Control plane version",
      nodeCount: "Nodes",
      nodePools: "Node pools",
      nodePoolsEmpty: "No node pools reported.",
      poolNodes: "{count} nodes",
      autopilot: "Autopilot",
      standard: "Standard",
      privateEndpoint: "Private control plane",
      publicEndpoint: "Public control plane",
      statuses: {
        running: "Running",
        degraded: "Degraded",
        provisioning: "Changing",
      },
      workloadsTitle: "Workloads are not listed",
      workloadsBody:
        "TaskTrooper cannot reach this cluster's control plane from outside your network, so it does not list what runs on it. This is a note, not a failure.",
    },
    loadFailed: "Could not load the Google Cloud resource",
  },
};
