...
vi.mock("@/components/projects/hosting/HostingPanel", () => ({
  HostingPanel: ({ subProjectPath, scopeKind }: { subProjectPath?: string; scopeKind?: string }) => (
    <div data-testid="hosting" data-scope={subProjectPath ?? ""} data-kind={scopeKind ?? ""} />
  ),
}));
...
describe("DeploySettingsSection scope", () => {
  ...
  it("starts on the repository itself and hands that scope to both panels", async () => {
    renderSection();

    expect(await screen.findByTestId("hosting")).toHaveAttribute("data-scope", "");
    expect(screen.getByTestId("targets")).toHaveAttribute("data-scope", "");
  ...
  it("moves the addresses to the sub-project the scope names", async () => {
    ...
    expect(screen.getByTestId("hosting")).toHaveAttribute("data-scope", "backend");
  });

  // A mobile sub-project ships through a store console, so that scope gets the
  // store panel where a hosted scope gets the hosting one.
  it("swaps hosting for the store panel on a mobile sub-project", async () => {
    renderSection();
    fireEvent.click(await screen.findByLabelText(/scope/i));
    ...
    expect(screen.queryByTestId("hosting")).not.toBeInTheDocument();
