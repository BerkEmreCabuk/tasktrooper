...
func (s *TaskAgentRunStoreSuite) TestTaskNumbersAreMonotonicPerType() {
	first, err := s.tasks.NextTaskNumber(s.ctx, "bug")
	s.Require().NoError(err)
	second, err := s.tasks.NextTaskNumber(s.ctx, "bug")
	s.Require().NoError(err)
	s.Equal(first+1, second)

	// A different type counts on its own, from its own floor.
	analiz, err := s.tasks.NextTaskNumber(s.ctx, "analiz")
	s.Require().NoError(err)
	analizAgain, err := s.tasks.NextTaskNumber(s.ctx, "analiz")
	s.Require().NoError(err)
	...
	afterDelete, err := s.tasks.NextTaskNumber(s.ctx, "bug")
	s.Require().NoError(err)
