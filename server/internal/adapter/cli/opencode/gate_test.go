...
// One session's rate limit must gate every OTHER session on this executor,
...
func TestQuotaGateParksWithoutSpawning(t *testing.T) {
	...
	block := &domain.QuotaBlock{ResumeAt: now.Add(30 * time.Minute), Detail: "rate limit exceeded"}
	...
	assert.Equal(t, domain.LLMProviderOpencode, gated.Provider)

	ex.clearQuotaGate()
	assert.Nil(t, ex.gatedQuotaBlock(req), "a cleared gate must stop parking")
}

// The gate only ever extends, never shortens.
