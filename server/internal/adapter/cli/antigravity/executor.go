...
// Executor runs tasks on agy. The whole spawn/parse/finish flow, the
...
var familySpec = core.FamilySpec{
	...
	NewError:        "antigravity executor: %w",
	NotConfigured:   "antigravity executor is not configured",
	NoWorkDir:       "antigravity executor: no task workspace to run in",
	StartFmt:        "start agy: %w",
	StdoutFmt:       "antigravity stdout: %w",
	GateLogMsg:      "antigravity quota gate is armed; parking without spawning",
	GateDetailFmt:   "another Antigravity session hit the quota: %s",
	QuotaLogMsg:     "antigravity quota reached, parking the task",
	BuildArgs:       buildArgs,
	Parse:           parseStream,
	ApplyMCP:        applyMCP,
	TraceStep:       "antigravity_session",
...
// buildArgs is the invocation contract with agy: print mode, apply everything
// unguarded, one model flag only when the agent chose one.
