...
// DefaultMaxConcurrent is how many cursor-agent sessions may run at once.
...
type Config struct {
	...
	Binary string
	// MaxConcurrent caps simultaneous cursor-agent sessions. <= 0 means
...
func (e *Executor) acquire(ctx context.Context) (func(), error) {
	...
	case e.slots <- struct{}{}:
		return func() { <-e.slots }, nil
	case <-timer.C:
		return nil, fmt.Errorf("cursor executor is busy: no session slot came free within %s (limit %d concurrent sessions)",
			wait, cap(e.slots))
	case <-ctx.Done():
		return nil, ctx.Err()
	}
