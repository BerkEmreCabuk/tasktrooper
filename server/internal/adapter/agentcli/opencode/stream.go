...
// event is one line of `opencode run --format json`'s JSONL event stream
// (https://opencode.ai/docs/cli/: step_start, text, tool_use, step_finish,
// error).
type event struct {
...
// parseStream reads one opencode run's JSONL event stream.
//
// A "text" part carries the FULL text seen so far for that part id, not a
// delta (opencode's parts are mutable objects updated in place, unlike
// claudecode's explicit text_delta field) — see textAccum for how that is
// turned into the incremental chunks a live chat stream needs.
...
// textAccum tracks the current value of every text part by id and turns a
// full-snapshot update into the incremental chunk a chat stream needs.
