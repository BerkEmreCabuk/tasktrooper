...
// ModelsTimeout bounds `opencode models`, a catalog read the CLI answers
...
// exists), opencode documents and answers this directly: `opencode models`
// prints one "provider/model-id" line per model on stdout, so the picker can
// be a live query instead of a curated guess. There is no separate label —
// the id is already the whole of what the CLI reports — so LLMModelOption's
// Label is left empty and the web app falls back to the id, same as every
// HTTP provider's listing.
...
func ParseModels(out string) []domain.LLMModelOption {
	...
	for _, line := range strings.Split(out, "\n") {
		...
		opts = append(opts, domain.LLMModelOption{ID: id})
	}
	return opts
}
