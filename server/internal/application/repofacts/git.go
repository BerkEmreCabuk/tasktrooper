...
func collectGit(ctx context.Context, root string, f *Facts) {
	if _, err := exec.LookPath("git"); err != nil {
		f.Warnings = append(f.Warnings, "git is not on PATH; git workflow facts are missing")
		return
	}
	if out, err := git(ctx, root, "rev-parse", "--is-inside-work-tree"); err != nil || strings.TrimSpace(out) != "true" {
		f.Warnings = append(f.Warnings, "working copy is not a git repository")
		return
	}

	g := GitFacts{}
	g.DefaultBranch = defaultBranch(ctx, root)
	g.RemoteHost, g.RemoteSlug = parseRemote(gitLine(ctx, root, "remote", "get-url", "origin"))
	g.HeadSHA = gitLine(ctx, root, "rev-parse", "--short", "HEAD")
	if ts := gitLine(ctx, root, "log", "-1", "--format=%cI"); ts != "" {
		if parsed, err := time.Parse(time.RFC3339, ts); err == nil {
			g.HeadCommittedAt = parsed.UTC()
		}
	}
...
func branchConvention(ctx context.Context, root, defaultBranch string) (samples []string, pattern string) {
	...
	pattern += " — " + itoa(top.n) + " of " + itoa(total) + " recent branches"
...
func mergeStyle(ctx context.Context, root string) (style string, directToMain bool) {
	...
	case merges*4 >= total:
		return "merge commits (" + itoa(merges) + " of last " + itoa(total) + ")", false
	default:
		return "mostly linear (" + itoa(merges) + " merge commits in last " + itoa(total) + ")", false
	}
}
...
func commitStyle(ctx context.Context, root string) string {
	...
	if conventional*2 >= len(lines) {
		return "Conventional Commits (" + itoa(conventional) + " of last " + itoa(len(lines)) + " subjects)"
	}
	return "free-form subjects (" + itoa(conventional) + " of last " + itoa(len(lines)) + " are conventional)"
}

func hotspots(ctx context.Context, root string) []Hotspot {
	out := gitOut(ctx, root, "log", "-"+itoa(churnCommits), "--name-only", "--format=")
	...
	ranked := make([]Hotspot, 0, len(counts))
	for p, n := range counts {
		if n < 3 {
			continue
		}
		ranked = append(ranked, Hotspot{Path: p, Commits: n})
	}
	sort.Slice(ranked, func(i, j int) bool {
