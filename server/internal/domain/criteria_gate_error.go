...
// gate: criteria left open (never ticked or cancelled), left unreviewed, or
...
// CriteriaGateCriterion is one blocking criterion, carried without the id so a
// human-facing rendering never has to strip one back out.
type CriteriaGateCriterion struct {
	ID   uuid.UUID
	Text string
}

// CriteriaGateError is what criteriaGate/criteriaReviewGate return when a move
// is refused for unsettled acceptance criteria.
//
// Error() stays the sentence those gates always produced — ids included,
// because the agent reading a tool refusal needs one to call review_criterion
// or set_criterion_completed. Target/Reason/Criteria are the same refusal
// broken into fields for a caller (the HTTP layer, moves made by a human) that
// must say the same thing without a raw id in it.
